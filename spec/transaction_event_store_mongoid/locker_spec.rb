...
RSpec.describe TransactionEventStoreMongoid::Locker do
  let(:timeout) { 1 }
  ...
  describe "#with_lock" do
    ...
    context "cannot obtain lock" do
      ...
      context "within the timeout" do
        it "retries" do
          expect(adapter).to receive(:with_lock).and_raise(TransactionEventStore::CannotObtainLock).ordered
          expect(adapter).to receive(:with_lock).and_raise(TransactionEventStore::CannotObtainLock).ordered
          expect(adapter).to receive(:with_lock).and_raise(TransactionEventStore::CannotObtainLock).ordered
      ...
      context "when the timeout expires" do
        it "raise the CannotObtainLock error" do
          ...
          expect(adapter).to receive(:with_lock).and_raise(TransactionEventStore::CannotObtainLock).once

          expect {
            locker.with_lock(stream, &block)
          }.to raise_error(TransactionEventStore::CannotObtainLock)
        end
      end
